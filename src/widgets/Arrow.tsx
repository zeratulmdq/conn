...
class Arrow extends React.PureComponent<PropTypes> {
  connectionDot = (x: number, y: number, isStart?: boolean) => {
  ...
  pathGenerator = (points: Point[], chartBranch: ChartBranch | null) => {
    ...
    if((isHorizontalStart && start.y === end.y) ||
      (!isHorizontalStart && start.x === end.x)) {
        ...
        return [
          this.connectionDot(start.x, start.y, true),
          <path d={d} stroke="black" strokeWidth="2" fill="none" />,
          this.connectionDot(end.x, end.y),
        ];
    }
    ...
    if(isHorizontalStart !== isHorizontalEnd) {
      ...
      return [
        this.connectionDot(start.x, start.y, true),
        <path d={d1} stroke="black" strokeWidth="2" fill="none" />,
        <path d={d2} stroke="black" strokeWidth="2" fill="none" />,
        this.connectionDot(end.x, end.y),
      ];
    }
    ...
    return [
      ...
      <path d={d1} stroke="black" strokeWidth="2" fill="none" />,
      <path d={d2} stroke="black" strokeWidth="2" fill="none" />,
      <path d={d3} stroke="black" strokeWidth="2" fill="none" />,
      this.connectionDot(end.x, end.y),
    ];
  };
  ...
  render() {
    return (
      <svg
        ...
        className="Arrow"
      >
        {this.pathGenerator(this.props.widget.points, this.props.widget.chartBranch)}
      </svg>
    );
  }
}
